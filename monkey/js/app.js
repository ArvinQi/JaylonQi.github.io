...
tv = {
  ...
  create3DHTML: function(i, x, y, z, sw, sh) {
    ...
    o.animate = function() {
      ...
      /* ==== 3D coordinates ==== */
      var h, scale, w, xy, xz, yx, yz;
      x = this.point3D.x - tv.camera.x.position;
      y = this.point3D.y - tv.camera.y.position;
      this.point3D.z.move(this.point3D.z.target, this.point3D.z.target ? .15 : .08);

      /* ==== rotations ==== */
      xy = tv.angle.cx * y - (tv.angle.sx * this.point3D.z.position);
      xz = tv.angle.sx * y + tv.angle.cx * this.point3D.z.position;
      yz = tv.angle.cy * xz - (tv.angle.sy * x);
      yx = tv.angle.sy * xz + tv.angle.cy * x;

      /* ==== 2D transform ==== */
      scale = tv.camera.focalLength / (tv.camera.focalLength + yz);
      x = yx * scale;
      y = xy * scale;
      w = Math.round(Math.max(0, this.point3D.w * scale * this.point3D.sw));
      h = Math.round(Math.max(0, this.point3D.h * scale * this.point3D.sh));
...
onload = function() {
  var i, img, j, onresize, structure;
  onresize = tv.resize;

  /* ==== build grid ==== */
  img = document.getElementById('bankImages').getElementsByTagName('img');
  structure = [];
  i = -300;
  while (i <= 300) {
    j = -300;
